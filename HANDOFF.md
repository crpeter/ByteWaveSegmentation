...
The current fixture is `assets/IMG_2771.mov`, point 0.4912/0.4706; existing models are under `.temporal-runs/dog-05/models`. Give all setup/rerun commands directly in chat; the user explicitly rejected being sent to a branch README for commands. Keep `diagnose_initializer.py` targeted at the original v1 packages for reproducible diagnostics. Do not integrate the owned models into iPhone until the complete comparison passes.
