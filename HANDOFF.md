...
1. Have Cody rerun `Temporal/validate_export.py` for the v2 precision policy on the existing dog clip. The six state tests already passed. Give the exact command directly in chat using the existing Python 3.11 environment. Request `status.json`, stage `report.json` on failure, or the first traceback. Do not run tests/inference/builds on his behalf.
